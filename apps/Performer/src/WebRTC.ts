...
import { cyclicalSync, throwIfUndefined, timer } from "./util";
...
export async function makeCall(invitation, socketInstance: Socket, serverOffsets: RefObject<number[]>, oneWayOffsets: RefObject<number[]>, oneWayOffsetAverage: RefObject<number>) {
  ...
  try {
    ...
    dc.onopen = async (event) => {
      ...
      await timer(100);
      cyclicalSync(targetId, connection, socketInstance).then(({ oneWayOffset, serverOffset }) => {
        if (oneWayOffset !== undefined && serverOffset !== undefined) {
          serverOffsets.current.push(serverOffset);
...
export async function handleOffer(offer, socketInstance: Socket, serverOffsets: RefObject<number[]>, oneWayOffsets: RefObject<number[]>, oneWayOffsetAverage: RefObject<number>) {
